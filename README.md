...
点击“预览字幕样式”会从第一条字幕的位置生成一张预览图，不会修改视频。预览尺寸可选择小（最大 720×405）、中（最大 960×540，默认）或大（最大 1280×720）；画面保持原视频宽高比，小分辨率视频不会被强制放大。该选项只影响预览 PNG，正式烧录仍保持原视频分辨率。预览窗口中的“刷新”会按当前尺寸和样式随机跳到后面 5–10 条字幕并重新渲染一帧，方便检查不同对白和画面上的效果；若某种字体仍显示异常，请从下拉列表换用 `Noto Sans CJK SC`、`SimHei`、`Meiryo` 或输入电脑上已安装的字体名称后再预览。
