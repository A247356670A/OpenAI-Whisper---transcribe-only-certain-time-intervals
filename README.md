...
完整识别与补全模式默认启用“B 音频预筛”：在 A 反向切出的 B 候选片段送入 Whisper 前，程序会跳过接近完全静音的片段，从源头减少空白片段产生的幻觉字幕。默认“仅跳过完全静音”使用较低阈值，以尽量保留轻声、远处对白和较安静的直播片段；可切换“平衡”“强过滤”预设，或直接调整 RMS 下限、最少有效音频时长、静音判定 dB。
