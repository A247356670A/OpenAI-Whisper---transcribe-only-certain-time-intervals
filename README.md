...
完整识别与补全模式默认不启用“B 音频预筛”：此时会完整保留按字幕 A 时间轴反向剪裁出的所有 B 片段，与原始逻辑一致。勾选后，程序会跳过接近完全静音的片段，从源头减少空白片段产生的幻觉字幕；“仅跳过完全静音”使用较低阈值，以尽量保留轻声、远处对白和较安静的直播片段。可切换“平衡”“强过滤”预设，或直接调整 RMS 下限、最少有效音频时长、静音判定 dB。

音量本身无法可靠地区分对白与 BGM。需要优先识别对话时，可勾选“语音优先”：它会使用 WebRTC VAD 跳过未检测到人声的 B 片段，适合减少纯音乐/环境音产生的幻觉，但可能漏掉音质很差、多人重叠或带强 BGM 的对白，因此默认关闭。该选项需要重新执行一次 `python -m pip install -r requirements.txt` 安装语音检测依赖；“宽松”通常最适合希望少漏对白的场景。
